...
export class Set3Page implements OnInit {
  foodArr;
  constructor(
    ...
    private firestore: AngularFirestore
      ...
      this.firestore.collection('Food').valueChanges({idField: 'id'}).subscribe((food) => {
        this.foodArr = food;
  ...
  game(){
    ...
    for (let i=0; i < 12; i++){
      ...
      cards[i].innerText = this.foodArr[i].id;
    }
    ...
    for (let j=0; j < 12; j++){
      ...
      cards[j].addEventListener('click', function () {
        ...
        if (numClicked == 2){
          ...
          for (let i=0; i < 12; i++){
            if (foodArr[i].id == jText){
              jTextMeaning = foodArr[i].meaning;
            }
            else if (foodArr[i].id == lastClickedText){
              lastClickedTextMeaning = foodArr[i].meaning;
            }
          }
